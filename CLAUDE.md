...
**The conflict resolver runs per-path, not as one `xargs`** — and that matters. Deleting `ci.yml` here while upstream keeps editing it makes a **modify/delete** conflict, which has no stage-2 ("ours") blob, so `git checkout --ours` fails with *"does not have our version"*. Under `bash -e` that single error used to abort the whole pass **before resolving any of the other conflicts**, and the sync produced no PR at all (Aug 2026, MSP-2.0#116: 11 conflicts, 10 of them routine, all left unmerged). The resolver now tests `git show :2:<path>` per file and `git rm`s the ones with no our-side version — for a path this fork deletes, staying deleted *is* our version. Any future path the fork deletes is covered by the same branch.
...
**Conflict handling — important:** when an upstream change conflicts with desktop's version, the workflow auto-resolves by **keeping the desktop version** and silently drops the upstream change for that file. The PR body lists the conflicted files under "Merge conflicts were auto-resolved". Three failure modes:
...
- **No PR at all** (nothing to review, easy to miss): see the per-path resolver note above. If a web push produced no sync PR, check the run log for *"does not have our version"* before assuming there was nothing to sync.

**Not every auto-resolved conflict is a drop to repair.** Sometimes keeping the desktop version is simply correct, because upstream's change assumes there is no desktop. `src/components/admin/AdminPage.tsx` is the standing example: MSP-2.0#116 deleted the `isTauri()` branch that skips the NIP-07 extension check, which is load-bearing here — desktop has no extension, so `!hasExtension` is permanently true and the admin page would be stuck on "no extension". Read what a change *assumes* before porting it; the useful move there was a hybrid (take upstream's `isLoading` gate and derived `authState`, keep the desktop bypass), not either side whole.

**Lint divergence, in both directions.** Desktop's Lint job is blocking; the web repo's was `continue-on-error` behind a 29-problem backlog until MSP-2.0#116 cleared it and made Lint blocking there too. So the old failure mode — upstream style that desktop refuses (an `as any`, a `useEffect` before its declared callees under `react-hooks/immutability`) — should now be caught upstream. Keep fixing that class **in the web repo** anyway (a pure reorder or a typed cast is a no-op there) so the clean file flows down instead of needing a re-patch each sync.

The reverse case now exists too: desktop sets `'react-hooks/set-state-in-effect': 'off'` in `eslint.config.js`, so upstream's `// eslint-disable-next-line react-hooks/set-state-in-effect` comments arrive **dead** and lint reports them as *"Unused eslint-disable directive"* warnings (currently 2, in `InfoIcon.tsx` and `PublisherEditor/PublishSection.tsx`). `eslint .` exits 0 on warnings so CI stays green, but the standing goal is 0 errors **and** 0 warnings. The durable fix is dropping desktop's rule override now that upstream annotates the call sites — not deleting the directives, which the next sync would restore.
